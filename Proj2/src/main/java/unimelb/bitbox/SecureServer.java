		...
		try {
			...
			while(true) {
				...
				case "LIST_PEERS_REQUEST":
					...
					break;
				case "CONNECT_PEER_RESQUEST":
					HostPort peerConnect = new HostPort(request.getString("host"), request.getInteger("port"));
					if(PeerMaster.containPeer(peerConnect)) {
						// peer already connected
						response = Protocol.DISCONNECT_PEER_RESPONSE(peerConnect, true, "connected to peer");
					} else {
						// try connect to peer
						if(PeerMaster.mode.equals("tcp")) {
							// connect for TCP
							TCPClient.peerToConnect.offer(peerConnect);
						} else {
							// connect for UDP
				...
				case "DISCONNECT_PEER_REQUEST":
					HostPort peerDisconnect = new HostPort(request.getString("host"), request.getInteger("port"));
